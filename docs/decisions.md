...
**Decision:** The local working directory at `/Users/USER/ai-sandbox/airlock-warlock` will be renamed to `/Users/USER/ai-sandbox/whizzard`. The rename is executed between Claude Code sessions (not mid-session), to avoid breaking the current session's working-directory binding. Steps: end current Claude session; remove the `crazy-ellis-b21769` worktree (`git worktree remove`); rename the parent directory; start a new Claude Code session pointed at the new path.

**Rationale:** D-03 prohibited mid-session rename for binding-stability reasons; that constraint doesn't apply between sessions. Consolidates the local layout with D-144 and D-145. After rename, Claude Code's auto-memory will live at a new encoded path (`-Users-USER-ai-sandbox-whizzard/`); the user must copy memory files from the old path or recreate them.
...
**Source:** /Users/USER/.claude/projects/-Users-USER-ai-sandbox-airlock-warlock/memory/feedback_mvp_focus.md (referenced from auto-memory)
...
- **2026-05-28 scope refinement:** the original v0.1.0 scope included Stages 16+17 (Discord control plane, read-only + write/approve). Both deferred to post-launch (v0.1.x or v0.2.0) to accelerate the OSS launch and plant the category flag earlier. Both stages are D-148 design-paused; the design conversation needs to land before code, which conflicts with the launch urgency surfaced by the 2026-05-28 strategic discussion (forward-deployed-engineer pitch to nanocoai team; inevitability of the agent-containment category). The maintainer accepts shipping v0.1.0 without their own daily-driver Discord control surface in exchange for the timing win.

**Source:** conversation 2026-05-27 (D-135 implementation-cost discussion surfaced the underlying question; resolved alongside the v0.1.0 framing); refined 2026-05-28 (strategic discussion on OSS launch timing and partnership-pitch dynamics).
...
- Three sub-questions surfaced as separate open decisions: D-174 (direct-mount + push-credentials interaction), D-175 (cache mount trust-shared surface), D-176 (constrained DNS resolver).
...
**Decision:** Before OSS-launch (D-131), the repo must meet a baseline security posture across six control categories: (a) account hardening — FIDO2 hardware-key 2FA, scoped/expiring PATs, quarterly OAuth-app audit; (b) branch protection on `main` — required PR review, dismiss-stale-on-new-commits, no admin bypass, signed-commit enforcement; (c) workflow hardening — third-party actions pinned to commit SHA, explicit `permissions:` blocks per workflow, no `pull_request_target` without safe checkout, no self-hosted runners on public PRs; (d) supply chain — lockfile review, Dependabot on, signed releases; (e) public disclosure — `SECURITY.md` at repo root with reporting channel and SLO; (f) PII / sensitive-string history scrub via `git filter-repo` while the repo is still private — catch-up review of 2026-05-23 found concrete disclosures (full PII paragraph in `docs/session_handoff.md`, the maintainer's macOS username in 15 places, a tailnet hostname); editing files only adds new state, so the scrub is the only way to remove these from history. Once the repo is public, history rewrite is effectively impossible because of forks/clones. Per-item status, priority, and verification commands live in `docs/internal/launch_readiness.md` (gitignored); the specific (f) items also live in `docs/known_issues.md` under "Pre-OSS-launch PII / history scrub" for visibility.
...
### D-174: Direct-mount + push-credentials interaction
...
**Notes:**
- Originally numbered D-173 on the `claude/rampart-clarity-whizzard-f4isl` branch; renumbered to D-174 at merge time (2026-05-27) because a parallel D-173 (Repo security hardening as OSS-launch blocker, above) had already landed on `main`. Branch commit 2b1fb9f still references the original D-173 in its message.

**Source:** conversation 2026-05-25 (D-135 design discussion; sub-question surfaced explicitly).

**Status:** open

### D-175: Cache mount trust-shared surface
...
**Notes:**
- Originally numbered D-174 on the `claude/rampart-clarity-whizzard-f4isl` branch; renumbered to D-175 at merge time (2026-05-27) — see D-174 note for context. Branch commit 2b1fb9f still references the original D-174 in its message.

**Source:** conversation 2026-05-25 (D-135 design discussion; sub-question surfaced explicitly).

**Status:** open

### D-176: Constrained DNS resolver
...
**Notes:**
- Originally numbered D-175 on the `claude/rampart-clarity-whizzard-f4isl` branch; renumbered to D-176 at merge time (2026-05-27) — see D-174 note for context. Branch commit 2b1fb9f still references the original D-175 in its message.

**Source:** conversation 2026-05-25 (network-exfil deep-dive; constrained-DNS option separated from the vault-gateway decision).

**Status:** open

### D-177: Persistent agent memory — scope, audit, and review semantics
...
**Notes:**
- Originally numbered D-176 on the `claude/rampart-clarity-whizzard-f4isl` branch; renumbered to D-177 at merge time (2026-05-27) — see D-174 note for context. Branch commit b054fd4 still references the original D-176 in its message.
...
**Decision:** Bump `HERMES_REF` in `Dockerfile.hermes` from `e8b9369` (untagged, 2026-05-28) to `3ef6bbd201263d354fd83ec55b3c306ded2eb72a` — the **v0.19.0 release**, git tag **v2026.7.20** (the repo uses date-based tags `vYYYY.M.D`; "v0.19.0" is the release name carried in that tag's commit). Adopted per D-189: smoke-gated. The target SHA and tag were verified against the upstream repo (not the handoff's label, which mislabeled the git tag).

The bump required a **build fix**: v0.19.0's dependency tree wants a different `packaging` than the one Debian preinstalls (via apt, no pip RECORD), and pip cannot uninstall a no-RECORD distribution — a *clean* build fails with `uninstall-no-record-file: Cannot uninstall packaging`. Fixed by adding `--ignore-installed` to the Hermes pip install (safe in this single-purpose container). Notably, a local rebuild passed on a **cached** layer and masked this; CI's clean build caught it — reinforcing [[feedback_smoke_real_path]]: a pin-bump smoke must run on a no-cache/CI build, not a cache-warm local one.
...
- **Phase C (backend wiring):** `firecrawl` is brokered through a **second broker instance** (reuse of the D-184 proxy pinned to `api.firecrawl.dev`, new `bearer_plain` auth scheme) that joins the cell's internal net — so the cell holds only a placeholder `FIRECRAWL_API_KEY` and the broker is its only route out; requires a mediated/hybrid net (validated, fail-loud). `ddgs` is keyless and un-brokerable (multi-host fan-out + `primp` TLS impersonation) → open-rung only, requires `open` egress, launch surfaces the expanded-surface notice. Both rungs ship in a `--search` cell image (`whiz hermes image build --search`) so the base image stays lean. Live smoke against a real Firecrawl key still pending (confirms Hermes's firecrawl backend reads `FIRECRAWL_API_URL`/`_KEY`).

**Source:** conversation 2026-07-24 (web-search build, Phases B–C; config-loading + web-search-mechanism investigations).

**Status:** active. Retires the D-167 manual-MCP-wiring step; closes `risk_register` §391 for authored leaves. Phase C backends (firecrawl broker + ddgs open-rung) wired; live firecrawl smoke pending.
