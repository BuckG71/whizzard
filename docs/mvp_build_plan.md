...
8. **Pre-publication PII / sensitive-string scrub of git history** (per D-173). The catch-up review of 2026-05-23 found concrete disclosures in tracked files: the maintainer's full PII (full name + personal email + GitHub handle) in `docs/session_handoff.md`, the macOS username `USER` across 15 occurrences in three docs, and a tailnet hostname in `docs/home_lab_deployment.md`. Editing files only adds new state — old commits remain reachable via `git log -p`. Use `git filter-repo` to retroactively scrub the agreed sensitive strings from history, then force-push, then verify `git log -p | grep` returns nothing. **Must happen while the repo is still private** — once public, forks/clones spread the old state and GitHub's reflog retains rewritten history for ~90 days. See `docs/known_issues.md` "Pre-OSS-launch PII / history scrub" for the specific item list and the maintainer's plan to move dev into a containerized environment going forward (option (c) in the catch-up review discussion).

9. **Public `ROADMAP.md` + README "Scope and limitations" section** (per D-131). The OSS launch ships as `v0.1.0` with v1.0 staked as a published post-launch milestone. `ROADMAP.md` (repo root) names the ten v1.0 primary goals from `docs/post_mvp_spec.md` with rough sequencing; the README "Scope and limitations" section honestly discloses known un-addressed classes (deferred-execution via writable-mount writes — mitigation roadmap'd to v1.0 `--strict-overlay` per D-135; DNS-based exfil — see D-176; etc.). Public commitment via `ROADMAP.md` substitutes for the launch-deadline pressure that drove MVP work, mitigating the "without urgency, v1.0 slips" risk identified in D-131.

Exit criteria: every invariant in architecture.md "Architectural Constants" has at least one adversarial test that fails if the invariant is broken; all launch-blocker findings resolved; CI runs the adversarial suite and the dependency scan on every push; git history scrub (deliverable 8) verified clean before any public visibility; `ROADMAP.md` + README "Scope and limitations" published with the launch (deliverable 9).
